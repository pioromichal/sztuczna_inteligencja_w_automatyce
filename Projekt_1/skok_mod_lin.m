function [t, h] = skok_mod_lin(tspan, h0, Tp, ...
    ...
    % Punkt pracy
    FDpp=14; F1pp=73; h2pp=15.6384; h1pp=h2pp*(alpha2/alpha1)^2;
    ...
    F1 = @(t) F1in(t - tau);
    F2 = @(h1) alpha1 * sqrt(h1_lin) + ...
        (alpha1 / (2 * sqrt(h1_lin))) * (h1 - h1_lin);
    F3 = @(h2) alpha2 * sqrt(h2_lin) + ...
        (alpha2 / (2 * sqrt(h2_lin))) * (h2 - h2_lin);
    FD = @(t) FDpp * (t <= 0) + ...
