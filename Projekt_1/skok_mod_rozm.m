function [t, h] = skok_mod_rozm(tspan, h0, Tp, F1in_vals, FD_vals, ...
    h2_lin_rozm, reg_switch_zak, varargin)
    % Parametry modelu
    ...
    % Zlinearyzowane równania modelu
    F2_lin = @(h1, h_l) alpha1 * sqrt(h_l) + ...
    (alpha1 / (2 * sqrt(h_l))) * (h1 - h_l);
    F3_lin = @(h2, h_l) alpha2 * sqrt(h_l) + ...
    (alpha2 / (2 * sqrt(h_l))) * (h2 - h_l);

    F2 = @(h1, mi) mi * arrayfun(@(i) ...
        F2_lin(h1, h1_lin_rozm(i)), 1:size(h1_lin_rozm, 1))';
    F3 = @(h2, mi) mi * arrayfun(@(i) ...
        F3_lin(h2, h2_lin_rozm(i)), 1:size(h2_lin_rozm, 1))';
    ...
    if nargin >=8 && varargin{1}==true
        odeSystemRozm = @(t, V) [
            dV1dt(t, V, ...
            fun_przyn_dzwon(h2(V(2)),h2_lin_rozm, reg_switch_zak,true));     % dV1/dt
            dV2dt(t, V, ...
            fun_przyn_dzwon(h2(V(2)),h2_lin_rozm, reg_switch_zak,true));     % dV2/dt
        ];
    else
        odeSystemRozm = @(t, V) [
            dV1dt(t, V, ...
            fun_przyn_trap(h2(V(2)),h2_lin_rozm, reg_switch_zak));     % dV1/dt
            dV2dt(t, V, ...
            fun_przyn_trap(h2(V(2)),h2_lin_rozm, reg_switch_zak));     % dV2/dt
        ];
    end
